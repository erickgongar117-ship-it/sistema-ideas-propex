...
export default async function PanoramaPage() {
  ...
  const metrics: ExecutiveMetric[] = [
    { id: "ideas", label: "Ideas", value: formatNumber(ideas.length), detail: `${openIdeas.length} abiertas · ${percent(closedIdeas.length, ideas.length)}% cerradas`, signal: `${overdueIdeas.length} vencidas`, color: "var(--brand-red)", href: "/dashboard" },
    { id: "kaizen", label: "Kaizen activos", value: formatNumber(activeKaizens.length), detail: `${kaizenProgress.percent}% de actividades resueltas`, signal: `${kaizenAtRisk} en riesgo`, color: "var(--brand-black)", href: "/kaizen" },
    { id: "genba", label: "GENBA abiertos", value: formatNumber(openGenbas.length), detail: `${genbaProgress.percent}% de acciones resueltas`, signal: `${averageAttendance}% asistencia`, color: "var(--maintenance)", href: "/genba" },
    { id: "training", label: "Participaciones", value: formatNumber(completedEnrollments.length), detail: `${trainingSessions.length} sesiones · ${trainingPrograms.filter((program) => program.active).length} programas`, signal: `${pendingPastTraining.length} por cerrar`, color: "var(--supervisor)", href: "/entrenamientos" },
    { id: "coins", label: "Saldo ProbocaCoins", value: formatNumber(coinBalance), detail: `${formatNumber(coinsAwarded)} otorgadas`, signal: `${formatNumber(coinsRedeemed)} canjeadas`, color: "var(--warning)", href: "/probocacoins" }
  ];

  const portfolio: ExecutivePortfolioRow[] = [
    { id: "ideas", label: "Ideas", status: `${openIdeas.length} abiertas`, progress: percent(closedIdeas.length, ideas.length), active: openIdeas.length, risk: overdueIdeas.length, detail: `${pendingInitialIdeas.length + pendingSupportReviews} decisiones pendientes`, href: "/dashboard", color: "var(--brand-red)" },
    { id: "kaizen", label: "Kaizen", status: `${activeKaizens.length} activos`, progress: kaizenProgress.percent, active: activeKaizens.length, risk: kaizenAtRisk, detail: `$${formatNumber(registeredSavings)} de ahorro registrado`, href: "/kaizen", color: "var(--brand-black)" },
    { id: "genba", label: "GENBA", status: `${openGenbas.length} abiertos`, progress: genbaProgress.percent, active: openGenbas.length, risk: genbaAtRisk, detail: `${promotedToKaizen} acciones enviadas a Kaizen`, href: "/genba", color: "var(--maintenance)" },
    { id: "training", label: "Entrenamientos", status: `${registeredEnrollments.length} inscritos`, progress: percent(completedEnrollments.length, enrollments.length), active: registeredEnrollments.length, risk: pendingPastTraining.length, detail: `${completedEnrollments.length} participaciones completadas`, href: "/entrenamientos", color: "var(--supervisor)" },
    { id: "coins", label: "ProbocaCoins", status: `${participantsWithMovements} personas`, progress: percent(participantsWithMovements, participants.filter((participant) => participant.active).length), active: participantsWithMovements, risk: negativeBalances, detail: `${coinTransactions.length} movimientos auditables`, href: "/probocacoins", color: "var(--warning)" }
  ];
