          ...
          {canManage ? (
            <details className="details-panel">
              ...
              <form action={updateGenbaWalkAction} className="grid gap-3 p-4">
                ...
                <fieldset><legend className="label">Asistencia</legend><div className="space-y-2">{genbaDepartments.map((department) => <div className="grid grid-cols-[1fr_70px_70px] items-center gap-2 rounded-lg border border-line p-2 text-xs" key={department}><span className="font-bold">{department}</span><label className="text-center"><span className="block text-[11px] uppercase text-slate-400">Esperado</span><input defaultChecked={expected.includes(department)} name="expectedDepartments" type="checkbox" value={department} /></label><label className="text-center"><span className="block text-[11px] uppercase text-slate-400">Asistió</span><input defaultChecked={attended.has(department)} name="attendedDepartments" type="checkbox" value={department} /></label></div>)}</div></fieldset>
