...
import { isWorkItemOverdue, kaizenLabel, workProgress } from "@/lib/domain";
                      ...
                      <div className="p-4">
                        <Link className="group flex items-start justify-between gap-3" href={`/kaizen/${project.id}`}>
                          <span className="min-w-0">
                            <span className="block text-[11px] font-extrabold uppercase text-amber-700">{kaizenLabel(project)}</span>
                        ...
                        {/* Con 23 de 40 proyectos sin plan capturado, el tablero se llenaba
                            de barras al 0% sobre "0 de 0". Una barra vacia no mide nada: dice
                            que no hay nada que medir, y ocupa lo mismo que una que si informa.
                            Cuando no hay actividades se dice que falta capturarlas. */}
                        {progress.total ? (
                          <div className="mt-4"><ProgressMeter label={`${progress.closed} de ${progress.total} activas realizadas`} percent={progress.percent} /></div>
                        ) : (
                          <p className="mt-4 rounded-lg bg-panel px-3 py-2 text-xs font-bold text-slate-600">Plan de acción por capturar</p>
                        )}
                      ...
                      <div className="border-t border-line bg-slate-50">
                        {activities.slice(0, 3).map((activity) => (
                          <Link className="grid grid-cols-[26px_minmax(0,1fr)_auto] items-start gap-2 border-b border-line px-3 py-3 transition last:border-0 hover:bg-white" href={`/kaizen/${project.id}#actividad-${activity.id}`} key={activity.id}>
                            <span className={`mt-0.5 flex h-6 w-6 items-center justify-center rounded-md text-[11px] font-extrabold ${["COMPLETADA", "CANCELADA"].includes(activity.status) ? "bg-emerald-100 text-emerald-800" : activity.status === "BLOQUEADA" ? "bg-rose-100 text-rose-800" : "bg-white text-slate-700"}`}>{activity.status === "COMPLETADA" ? <CheckCircle2 className="h-3.5 w-3.5" aria-hidden /> : activity.number}</span>
                            <span className="min-w-0">
                              <span className="line-clamp-2 block text-xs font-extrabold leading-4 text-slate-800">{activity.action}</span>
                              <span className={`mt-1 block truncate text-[11px] font-bold ${isWorkItemOverdue(activity) ? "text-rose-700" : "text-slate-500"}`}>{activity.owner?.name ?? "Sin responsable"} / {activity.dueDate ? activity.dueDate.toLocaleDateString("es-MX") : "Sin fecha"}</span>
                            </span>
                            <WorkStatusPill status={activity.status} />
                          </Link>
                        ))}
                        {!activities.length ? <p className="px-4 py-4 text-center text-xs font-bold text-slate-500">Cuando se capturen, las primeras tres se listan aquí.</p> : null}
